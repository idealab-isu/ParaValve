'''
Simulate blood flow, arterial wall deformation, and valve deformation of
immersed bio-prosthetic heart valves.

Notes:
 - This script is designed to be run after the Preprocessing script is run.
 - Preprocessing.py identifies and saves the interior facets within the solid
   domain (arterial wall) & calculates the intersection points between the
   stent and the arterial wall and saves them as a mesh function.
 - This script relies on FEniCS and custom libraries such as VarMINT,
   SalaMANdER, CouDALFISh, tIGAr, and ShNAPr.
 - Input Data: Pressure waveform data and valve geometry files in smesh format.
 - Output Data: Fluid-solid mesh results, shell results, and
   quantity of interest (QOI) data in CSV format.
 - Directory Structure: Refer to the default arguments for input files
   (meshes, valve geometries, pressure waveforms) and output files (results, restarts).
 - It is recommended that this script be run first on a coarse mesh in serial to
   JIT-compile the weak forms. Be aware, the forms for the valve computations
   take a long time to compile and require a great deal of memory.
 - The results are best visualized with a single slice in the fluid domain.
   Sample Paraview state files are available for guidance.
 - This script is designed for parallel execution and is suitable for 
...
# constants
DEFAULT_LEAFLET_BC_EDGES = [[0,0],[0,1],[1,1]]  # default edges for leaflet BCs [direction, side]
...
p.add_argument("--outfile-shell-results",dest="FILE_SHELL_DISP",
               ...
               help="The name of the shell displacement results file.")
...
p.add_argument("--valve-folder",dest="VALVE_FOLDER",
               default="./valves/BHV-23mm-finestent/",
...
p.add_argument("--mesh-nu",dest="MESH_NU",
               ...
               help="The fictitious Poisson's ratio assigned to the material.")
...
###############################################################
#### Stent-solid intersections ################################
###############################################################
log("  Loading stent-solid intersections from file.")
mvc = MeshValueCollection('size_t', mesh, nsd-1)
...
dX = dy(FLAG["solid"])
solid_alpha = SOLID["alpha"]    # penalty parameter 1e3
beta = (solid_alpha*SOLID["E"]) / (h_min_cell*(1-SOLID["nu"]**2)) # penalty coefficient
solid_kappa = sm.bulkModulus(SOLID["E"],SOLID["nu"]) # bulk modulus
solid_mu = sm.shearModulus(SOLID["E"],SOLID["nu"]) # shear modulus
...
res_s += solid_model.penaltyDirichletBCResidual(u=dot(u_s_alpha,n)*n,
                                                v=dot(dv,n)*n,
                                                g=zero,beta=beta,
...
res_f += vrmt.stableNeumannBC(-Q*FLUID_NUMERICAL["R"]*n,FLUID["rho"],
                                ...
                                gamma=Constant(0))
"""
TODO: Adding RCR windkessel model to the fluid problem.
"""
...
power = MESH["power"]       # power for Jacobian stiffening 1.0
fictitious_E = MESH["E"]    # fictitious Young's modulus 1.0
fictitious_nu = MESH["nu"]  # fictitious Poisson's ratio 0.3
mesh_alpha = MESH["alpha"]  # penalty parameter 1e3
beta = mesh_alpha*(fictitious_E/((J_M/J_M_0)**power)) \
       / (h_min_cell*(1-fictitious_nu**2)) # penalty coefficient

mesh_model = sm.JacobianStiffening(power=Constant(1.0))
res_m = mesh_model.interiorResidual(uhat_alpha,duhat,dx=dy)
res_m += mesh_model.penaltyDirichletBCResidual(u=dot(uhat_alpha,n)*n,
                                               v=dot(duhat,n)*n,
                                               g=zero,beta=beta,
                                               ds=ds(FLAG["fluid_outflow"]))
...
log("Build XDMF file to store velocity, pressure, & uhat.")
...
for timeStep in range(startStep, NUM_TIME_STEPS):
    ...
    if WRITE_VIS:
        ...
        if timeStep%WRITE_VIS_SKIP_SHELL==0:
            ...
            # Curvilinear basis vectors and metric tensor
            A0,A1,_,deriv_A2,A,B = surfaceGeometry(spline,X)
            _,_,_,_,a,b = surfaceGeometry(spline,X+y)
        ...
        if timeStep%WRITE_VIS_SKIP_FLUID_SOLID==0:
            log("Writing fluid-solid-mesh visualization files.")
    ...
    # write the fluid flow statistics
